...
[[ $(type -t GplIspconfigControlManageEmailMailbox_printHelp) == function ]] || GplIspconfigControlManageEmailMailbox_printHelp() {
    cat << EOF
GPL ISPConfig Control
Variation Manage Email Mailbox
Version `GplIspconfigControlManageEmailMailbox_printVersion`

EOF
    cat << 'EOF'
Usage: gpl-ispconfig-control-manage-email-mailbox.sh [options]
...
# Help and Version.
[ -n "$help" ] && { GplIspconfigControlManageEmailMailbox_printHelp; exit 1; }
[ -n "$version" ] && { GplIspconfigControlManageEmailMailbox_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplIspconfigControlManageEmailMailbox_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
# Title.
title GPL ISPConfig Control
_ 'Variation '; yellow Manage Email Mailbox; _.
_ 'Version '; yellow `GplIspconfigControlManageEmailMailbox_printVersion`; _.
____
