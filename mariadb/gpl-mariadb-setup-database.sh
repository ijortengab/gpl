...
[[ $(type -t GplMariadbSetupDatabase_printHelp) == function ]] || GplMariadbSetupDatabase_printHelp() {
    cat << EOF
GPL MariaDB Setup Database
Variation Default
Version `GplMariadbSetupDatabase_printVersion`
...
# Help and Version.
[ -n "$help" ] && { GplMariadbSetupDatabase_printHelp; exit 1; }
[ -n "$version" ] && { GplMariadbSetupDatabase_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplMariadbSetupDatabase_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
# Title.
title GPL MariaDB Setup Database
_ 'Variation '; yellow Default; _.
_ 'Version '; yellow `GplMariadbSetupDatabase_printVersion`; _.
____
