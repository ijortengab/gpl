...
[[ $(type -t GplCertbotDigitaloceanAutoinstaller_printHelp) == function ]] || GplCertbotDigitaloceanAutoinstaller_printHelp() {
    cat << EOF
GPL Certbot DigitalOcean Autoinstaller
Variation Default
Version `GplCertbotDigitaloceanAutoinstaller_printVersion`

EOF
    cat << 'EOF'
Usage: gpl-certbot-digitalocean-autoinstaller.sh [options]
...
# Requirement.
command -v "snap" >/dev/null || { echo -e "\e[91m" "Unable to proceed, snap command not found." "\e[39m"; exit 1; }
...
# Title.
title GPL Certbot DigitalOcean Autoinstaller
