...
[[ $(type -t GplDigitaloceanApiManageDomain_printHelp) == function ]] || GplDigitaloceanApiManageDomain_printHelp() {
    cat << EOF
GPL DigitalOcean API
Variation Manage Domain
Version `GplDigitaloceanApiManageDomain_printVersion`

EOF
    cat << 'EOF'
Usage: gpl-digitalocean-api-manage-domain.sh [command] [options]
...
# Help and Version.
[ -n "$help" ] && { GplDigitaloceanApiManageDomain_printHelp; exit 1; }
[ -n "$version" ] && { GplDigitaloceanApiManageDomain_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplDigitaloceanApiManageDomain_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
# Title.
title GPL DigitalOcean API
_ 'Variation '; yellow Manage Domain; _.
_ 'Version '; yellow `GplDigitaloceanApiManageDomain_printVersion`; _.
____
