...
Version `GplWslSetupLempStack_printVersion`

EOF
    cat << 'EOF'
Usage: gpl-wsl-setup-lemp-stack.sh [options]
...
# Help and Version.
[ -n "$help" ] && { GplWslSetupLempStack_printHelp; exit 1; }
[ -n "$version" ] && { GplWslSetupLempStack_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplWslSetupLempStack_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
[[ $(type -t GplWslSetupLempStack_makeSureRunning) == function ]] || GplWslSetupLempStack_makeSureRunning() {
    local service="$1"
...
_ 'Version '; yellow `GplWslSetupLempStack_printVersion`; _.
____
...
if [ -n "$wsl" ];then
    GplWslSetupLempStack_makeSureRunning nginx
    GplWslSetupLempStack_makeSureRunning mariadb
    GplWslSetupLempStack_makeSureRunning "php${php_version}-fpm"
fi
____
