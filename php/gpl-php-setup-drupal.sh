...
GPL PHP Setup
Variation Drupal
Version `GplPhpSetupDrupal_printVersion`
...
# Help and Version.
[ -n "$help" ] && { GplPhpSetupDrupal_printHelp; exit 1; }
[ -n "$version" ] && { GplPhpSetupDrupal_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplPhpSetupDrupal_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `GplPhpSetupDrupal_printVersion`; _.
____
