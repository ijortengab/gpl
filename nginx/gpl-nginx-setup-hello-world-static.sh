...
# Functions.
[[ $(type -t GplNginxSetupHelloWorldStatic_printVersion) == function ]] || GplNginxSetupHelloWorldStatic_printVersion() {
    echo '0.1.0'
}
[[ $(type -t GplNginxSetupHelloWorldStatic_printHelp) == function ]] || GplNginxSetupHelloWorldStatic_printHelp() {
    cat << EOF
GPL Nginx Setup
Variation Hello World Static
Version `GplNginxSetupHelloWorldStatic_printVersion`

EOF
    cat << 'EOF'
Usage: gpl-nginx-setup-hello-world-static.sh [options]
...
# Help and Version.
[ -n "$help" ] && { GplNginxSetupHelloWorldStatic_printHelp; exit 1; }
[ -n "$version" ] && { GplNginxSetupHelloWorldStatic_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplNginxSetupHelloWorldStatic_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `GplNginxSetupHelloWorldStatic_printVersion`; _.
____
...
INDENT+="    ";
source $(command -v gpl-nginx-setup-static.sh)
INDENT=${INDENT::-4}
