...
# Prerequisite.
[ -f "$0" ] || { echo -e "\e[91m" "Cannot run as dot command. Hit Control+c now." "\e[39m"; read; x 1; }
...
[[ $(type -t GplDrupalAutoinstallerNginxPhpFpm_printHelp) == function ]] || GplDrupalAutoinstallerNginxPhpFpm_printHelp() {
    cat << EOF
GPL Drupal Auto-Installer
Variation Nginx PHP-FPM
Version `GplDrupalAutoinstallerNginxPhpFpm_printVersion`
...
# Help and Version.
[ -n "$help" ] && { GplDrupalAutoinstallerNginxPhpFpm_printHelp; x 1; }
[ -n "$version" ] && { GplDrupalAutoinstallerNginxPhpFpm_printVersion; x 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplDrupalAutoinstallerNginxPhpFpm_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
[[ $(type -t x) == function ]] || x() { echo >&2; x 1; }
...
# Title.
title GPL Drupal Auto-Installer
_ 'Variation '; yellow Nginx PHP-FPM; _.
_ 'Version '; yellow `GplDrupalAutoinstallerNginxPhpFpm_printVersion`; _.
____
...
_ -----------------------------------------------------------------------;_.;_.;
INDENT+="    "
source $(command -v gpl-nginx-setup-drupal.sh)
INDENT=${INDENT::-4}
_ -----------------------------------------------------------------------;_.;_.;
...
INDENT+="    "
source $(command -v gpl-mariadb-setup-database.sh)
INDENT=${INDENT::-4}
_;_, ____________________________________________________________________;_.;_.;
