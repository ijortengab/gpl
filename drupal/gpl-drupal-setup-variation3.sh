...
Version `GplDrupalSetupVariation3_printVersion`
...
# Help and Version.
[ -n "$help" ] && { GplDrupalSetupVariation3_printHelp; exit 1; }
[ -n "$version" ] && { GplDrupalSetupVariation3_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `GplDrupalSetupVariation3_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `GplDrupalSetupVariation3_printVersion`; _.
____
